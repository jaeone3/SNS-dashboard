 ...
 * Body: { platform: "instagram" | "facebook", accountId?: string }
 */
export async function POST(request: NextRequest) {
  try {
    const { platform, accountId = "default" } = (await request.json()) as { 
      platform: string; 
      accountId?: string;
    };
    ...
    // Open visible browser for login with User Data Directory
    await openLoginBrowser(platform, accountId);

    return NextResponse.json({
      status: "browser_opened",
      message: `${platform} login browser opened (account: ${accountId}). Please log in manually, then call DELETE to close.`,
    });
  } catch (error) {
