 ...
 * - TikTok:     Apify TikTok Profile Scraper (reliable, maintained)
...
// ============================================================
// Login session management (User Data Directory based persistence)
// Longer session lifetime (3-6 months base, 9-12+ months with keep-alive)
// ============================================================
const PROFILE_DIR = path.join(os.homedir(), ".sns-dashboard-profiles");

function getProfilePath(platform: string, accountId: string = "default"): string {
  return path.join(PROFILE_DIR, platform, accountId);
}

/** Check if a login session (browser profile) exists for a platform */
export function hasLoginSession(platform: string, accountId: string = "default"): boolean {
  const profilePath = getProfilePath(platform, accountId);
  // Check if profile directory exists and has essential session data
  return fs.existsSync(profilePath) && fs.readdirSync(profilePath).length > 0;
}

// Track open login browsers so we can close them
const _loginBrowsers: Record<
  string,
  { browser: Awaited<ReturnType<typeof puppeteer.launch>>; page: unknown; accountId: string }
> = {};

/**
 * Open a visible browser for manual login using User Data Directory.
 * The user logs in manually, then calls closeLoginBrowser() to finalize.
 * Browser profile (cookies + localStorage + IndexedDB) is automatically saved.
 */
export async function openLoginBrowser(platform: string, accountId: string = "default"): Promise<void> {
  ...
  const browser = await puppeteer.launch({
    headless: false,
    userDataDir: profilePath,  // KEY CHANGE: Use persistent profile directory
    args: [
      "--no-sandbox",
  ...
  _loginBrowsers[platform] = { browser, page, accountId };
  console.log(`[Auth] Opened ${platform} login browser (profile: ${accountId}). Please log in manually.`);
  console.log(`[Auth] Session data will be saved to: ${profilePath}`);
}

/**
 * Close the login browser. Profile data is auto-saved by userDataDir.
 */
export async function closeLoginBrowser(platform: string): Promise<void> {
  ...
  if (entry) {
    try {
      ...
      console.log(`[Auth] Saved browser profile for ${platform} (account: ${entry.accountId}).`);
      console.log(`[Auth] Profile location: ${getProfilePath(platform, entry.accountId)}`);
    } catch (e) {
      console.error(`[Auth] Error closing browser for ${platform}:`, e);
    }
...
// Track logged-in browsers to ensure proper cleanup
const _loggedInBrowsers: Record<string, Awaited<ReturnType<typeof puppeteer.launch>>> = {};

/**
 * Launch a headless stealth browser using saved User Data Directory.
 * Returns { page, browser } — caller must close both when done.
 */
async function openLoggedInPage(platform: string, url: string, accountId: string = "default") {
  const profilePath = getProfilePath(platform, accountId);
  if (!fs.existsSync(profilePath)) {
    throw new Error(`No login session for ${platform}. Please login first via /api/auth/login`);
  }

  // Close any existing browser for this profile to prevent conflicts
  const browserKey = `${platform}-${accountId}`;
  if (_loggedInBrowsers[browserKey]) {
    try {
      await _loggedInBrowsers[browserKey].close();
    } catch (e) {
      // Ignore close errors
    }
    delete _loggedInBrowsers[browserKey];
  }

  // Launch browser with persistent profile (cookies + localStorage + IndexedDB)
  const browser = await puppeteer.launch({
    headless: "new" as never,
    userDataDir: profilePath,  // KEY CHANGE: Reuse saved profile
    args: [
      "--no-sandbox",
      "--disable-setuid-sandbox",
      "--disable-infobars",
  ...
  _loggedInBrowsers[browserKey] = browser;

  const page = await browser.newPage();
  await page.setViewport({ width: 1920, height: 1080 });

  // Navigate (session already loaded from profile)
...
// Shared stealth browser for Instagram/Facebook (lazy singleton)
...
/**
 * TikTok scraper using Apify official TikTok Scraper
 * API docs: https://apify.com/apify/tiktok-scraper
 */
export async function scrapeTikTok(username: string): Promise<ScrapeResult> {
  ...
  if (!apiKey) {
    console.error("[TikTok] APIFY_API_TOKEN not set in .env");
  ...
  try {
    ...
    // Call Apify official TikTok Scraper actor synchronously
    // Note: Actor ID uses ~ instead of / in API calls
    ...
    if (!response.ok) {
      ...
      console.error(`[TikTok] Apify API error: ${response.status} ${response.statusText}`);
      console.error(`[TikTok] Error details: ${errorBody}`);
    ...
    if (!data || data.length === 0) {
      console.warn(`[TikTok] No data returned for @${username}`);
      throw new Error(`No data returned for @${username}`);
    }

    // First item contains profile + latest video data
    const item = data[0];

    // Followers from authorMeta
    if (item.authorMeta?.fans != null) {
      result.followers = item.authorMeta.fans;
    } else if (item.authorMeta?.followers != null) {
      result.followers = item.authorMeta.followers;
    }

    // Video stats
    if (item.playCount != null) {
      result.lastPostView = item.playCount;
    }
    if (item.diggCount != null) {
      result.lastPostLike = item.diggCount;
    }

    // Video date
    if (item.createTimeISO) {
      result.lastPostDate = item.createTimeISO.split("T")[0];
    } else if (item.createTime) {
      result.lastPostDate = new Date(item.createTime * 1000).toISOString().split("T")[0];
    }
...
export async function scrapeInstagram(username: string): Promise<ScrapeResult> {
  ...
  try {
    ...
    const loginResult = await openLoggedInPage("instagram", `https://www.instagram.com/${username}/`);
...
export async function scrapeFacebook(username: string): Promise<ScrapeResult> {
  ...
  if (!hasLoginSession("facebook")) {
    ...
    try {
      ...
      try {
        ...
        console.debug(`[Facebook] Dialog close failed (expected if no dialog):`, e);
      }
  ...
  try {
    ...
    const loginResult = await openLoggedInPage("facebook", profileUrl);
    page = loginResult.page;
    browser = loginResult.browser;
